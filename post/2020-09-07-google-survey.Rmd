...
dates = seq.Date(as.Date(start_day), as.Date(end_day), by = "day")
geo_values_complete = intersect(intersect(df_in_act$geo_value,
                                          df_go_act$geo_value),
                                df_fb_act$geo_value)

y = df_in_act %>% select(time_value, geo_value, value) %>% 
  filter(geo_value %in% geo_values_complete)
x = df_go_act %>% select(time_value, geo_value, value) %>% 
  filter(geo_value %in% geo_values_complete)
z = df_fb_act %>% select(time_value, geo_value, value) %>% 
  filter(geo_value %in% geo_values_complete)
...
Lastly, let's consider the differences between the surveys. Here's a summary:
