...
dissemination (well beyond what we could do onurselves). 
...
to run questionnaires). It asked just a single question: 
...
the US. After about 2 weeks, we paused the survey.
One concern on Google's side was whether we can get equally useful
information without asking a question of such a sensitive nature.
In general, asking a person about their health (or their family's health) 
is not common practice on Google's survey platform.
...
survey. In both plots, we see a reassuring trend, 
...
CLI-in-community question slightly differently, and the Facebook survey asks
about an additional symptom (sore throat); they reach different subpopulations 
of the US; and in fact, the estimation procedures behind the surveys handle 
missingness differently. But as far as we can tell, none of this can really 
explain why the Facebook numbers are over twice as large as the Google ones, a 
trend that is pretty consistent across location and time. We'll save further 
analysis for when we work on deploying these two surveys in tandem 
(understanding the differences in scale would be a critical step before we could
draw any joint inferences). But for now, we'll emphasize that this observation 
reiterates the importance of focusing on *time-varying trends* in the survey 
signals, and not the signal values themselves (a point we [made in our last 
post](https://delphi.cmu.edu/blog/2020/08/26/covid-19-symptom-surveys-through-facebook/#why-run-these-surveys)). 
...
platform*; this means that we receive all the individual survey responses 
directly (and Facebook never sees any of the data).
...
that provides answers to a [rich set of questions](TODO: link to full question
coding?)
...
or manually (in collaboration with partners in public health or data 
journalists). Either way, we have our work cut out for us!
TODO: scrap or change last line?

**Credits.** *Ryan Tibshirani wrote the initial code for producing estimates
from the aggregated survey data. Sangwon Hyun, Natalia Lombardi de Oliveira, and
Lester Mackey greatly extended and improved this codebase, and they developed,
along with Ryan Tibshirani, the underlying statistical methodology. Ryan 
Tibshirani came up with the idea of running the surveys, and worked with Google 
to make this a reality. On the Google side, Brett Slatkin and Hal Varian have 
been key collaborators; Brett Slatkin wrote the code to get daily survey data 
over to Delphi's estimation pipeline; and both contributed numerous important 
ideas at various stages of the project.*
