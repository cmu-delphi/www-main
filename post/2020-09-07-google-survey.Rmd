...
Working with Brett Slatkin (engineering lead of Google Surveys) 
...
# Find counties present in all three data signals
geo_values_complete = intersect(intersect(df_go_act$geo_value,
                                          df_fb_act$geo_value),
                                df_in_act$geo_value)

# Append 1-2 week lags to (log-transformed) Google, Facebook signals
lags = -1:-2 * 7
g = df_go_act %>% select(geo_value, time_value, value) %>%
  filter(geo_value %in% geo_values_complete) %>% 
  mutate(value = log(value + 1)) %>% 
  append_shifts(shifts = lags) 
f = df_fb_act %>% select(geo_value, time_value, value) %>%
  filter(geo_value %in% geo_values_complete) %>% 
  mutate(value = log(value + 1)) %>% 
  append_shifts(shifts = lags) 

# Append 1-2 week lags and 1-2 week leads to (log-transformed) cases
leads = 1:2 * 7
c = df_in_act %>% select(geo_value, time_value, value) %>%
  filter(geo_value %in% geo_values_complete) %>%
  mutate(value = log(value + 1)) %>% 
...
# Use quantgen for LAD regression (this package is much more general; you can
# find it here: https://github.com/ryantibs/quantgen)
library(quantgen) 
...
for (i in 1:length(leads)) { 
  ...
  for (j in 1:length(valid_dates)) {
    ...
    res_list[[i]][inds,]$err0 = abs(exp(y_hat) - exp(y_te))
    ...
    if (sum(ok) > 0) {
      ...
      res_list[[i]][inds,]$err1 = abs(exp(y_hat) - exp(y_te)) 
    }
    ...
    if (sum(ok) > 0) {
      obj = quantile_lasso(as.matrix(x_tr[ok,]), y_tr[ok], tau = 0.5,
                           lambda = 0, stand = FALSE, lp_solver = "gurobi")
      y_hat = as.numeric(predict(obj, newx = as.matrix(x_te)))
      res_list[[i]][inds,]$err2 = abs(exp(y_hat) - exp(y_te)) 
    }
    ...
    if (sum(ok) > 0) {
      obj = quantile_lasso(as.matrix(x_tr[ok,]), y_tr[ok], tau = 0.5,
                           lambda = 0, stand = FALSE, lp_solver = "gurobi")
      y_hat = as.numeric(predict(obj, newx = as.matrix(x_te)))
      res_list[[i]][inds,]$err3 = abs(exp(y_hat) - exp(y_te)) 
    }
    ...
    if (sum(ok) > 0) {
      obj = quantile_lasso(as.matrix(x_tr[ok,]), y_tr[ok], tau = 0.5,
                           lambda = 0, stand = FALSE, lp_solver = "gurobi")
      y_hat = as.numeric(predict(obj, newx = as.matrix(x_te)))
      res_list[[i]][inds,]$err4 = abs(exp(y_hat) - exp(y_te)) 
    }
  }
}
...
res_med = res %>% mutate(err1 = err1 / err0, err2 = err2 / err0,
  ...
  mutate(lead = factor(lead, labels = paste(leads, "days ahead")),
         model = factor(model, labels = c("Cases", "Cases + Google",
                                          "Cases + Facebook", 
...
```{r}
# Error table for 7 days ahead forecasts, all models
knitr::kable(res_med %>% 
               group_by(model, lead) %>% 
               filter(time_value <= max(g$time_value)) %>%
               filter(lead == "7 days ahead") %>% 
               rename(Model = model, Target = lead) %>% 
               summarize("Scaled error" = median(err, na.rm = TRUE),
                         "Test days" = sum(!is.na(err))))
```

blah

```{r}
# Error table for 7 and 14 days ahead forecasts, just models 1 and 3
knitr::kable(res_med %>% 
               group_by(model, lead) %>% 
               filter(model %in% c("Cases", "Cases + Facebook")) %>%
               rename(Model = model, Target = lead) %>% 
               summarize("Scaled error" = median(err, na.rm = TRUE),
                         "Test days" = sum(!is.na(err))) %>%
               arrange(Target))
...
ggplot(res_med %>% 
         filter(lead == "7 days ahead" | 
                  model %in% c("Cases", "Cases + Facebook")), 
...
along with Ryan, the underlying statistical methodology. Ryan came up with the 
idea of running the surveys, and worked with Google to make this a reality. On 
the Google side, Brett Slatkin and Hal Varian have been key collaborators; Brett
wrote the code to get daily survey data over to Delphi's estimation pipeline; 
and both contributed numerous important ideas at various stages of the project.*
