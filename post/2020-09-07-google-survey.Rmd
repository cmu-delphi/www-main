...
we've been running (even-more-massive) surveys through Google to 
track the spread of COVID-19 in the United States. 
At its peak, our Google survey was taken by over 1.2 million people in a single day, 
and over its first month in operation, averaged over 600,000 daily respondents. 
In mid May, we paused daily dissemination of this survey in order to focus 
on our (longer, more complex) survey through Facebook,
but we plan to bring back the Google survey this fall. 
This short post covers some key differences between our Google and Facebook surveys, 
explains the backstory behind the "CLI-in-community" question
...
has been to produce real-time, county-level data streams of self-reported COVID symptoms 
that can potentially serve as **early indicators** of COVID activity in the US. 
As we noted in that post, we weren't the only data scientists
who thought of running COVID-19 symptom surveys---several 
other groups had deployed surveys before us. 
What distinguished our strategy from others' 
was the pursuit of a giant like Google to achieve widespread and continuous dissemination 
(well beyond what we could do onurselves). 
Googles willingness to help was a *huge* win for us. 
...
[various ways ever since](TODO: hopefully we 
...
deployed through various websites and apps 
and it asked just a single question: 
...
the percentage of people with COVID-like illness, in over 1,000 counties across the US.
After about 2 weeks, we paused the survey.
One concern on Google's side was whether we can get equally useful information
without asking a question of such a sensitive nature.
In general, person health questions (or their family's health) 
are not common practice on Google's survey platform.

## CLI-in-Community 

Working with Brett Slatkin (head of Google Surveys) 
and Hal Varian (Google's Chief Economist), we looked for a new question. 
Brett came up with a list of questions that were passable,
...
the percentage of people who *know someone in their community* with COVID-like illness. 
The initial results far exceeded our expectations, 
...
health information, it turned our to deliver [much higher
correlations](https://delphi.cmu.edu/blog/2020/08/26/covid-19-symptom-surveys-through-facebook/#basic-correlation-analysis) 
than the direct (non-proxy) question. 

To give you a feel for this data, below we plot 
...
we reproduce this with the estimated % CLI-in-community from our Facebook survey. 
In both plots, we see a reassuring trend, 
but the trend on the left is noticeably stronger. 
Indeed, the correlation here between the Google signal and case rates is 0.84, 
while that between the Facebook signal and case rates is 0.64. 
...
day, with a target of at least 1,000 responses per county. 
...
visit our [COVIDcast signals documentation](https://cmu-delphi.github.io/delphi-epidata/api/covidcast-signals/google-survey.html).
...
We now discuss some similarities and differences 
between the Google and Facebook surveys.
Starting with similarities, both are have been deployed at a massive scale, 
...
This is no doubt encouraging, especially because 
the Google correlations would have likely only improved later in the year, 
as was the case with the Facebook correlations (which we [previously 
...
bias in the two surveys; in an absolute sense, 
the subsequent estimates of % CLI-in-community strongly disagree, 
so both can't be right, and this casts 
...
- This makes a big difference as to how much (and what questions) we can ask on the survey: 
our Google survey is just a single question long, and our Facebook 
...
As we can see from the above summary, the two survey schemes are complementary, 
and could be  used synergistically. 
Our Facebook survey is a continuously-running, wide-reaching instrument 
that provides answers to a rich set of questions 
...
it like a **survey microscope** (credit to Hal Varian for inventing the terminology!).
This could be done automatically 
(would be a pretty hard, nonstationary multi-armed bandit problem) 
or manually (in collaboration with partners in public health or data journalists).
