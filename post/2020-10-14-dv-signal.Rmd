---
title: "Using Electronic Health Records to Develop Real-Time COVID-19 Indicators"
author: "Aaron Rumack"
date: 2020-10-14
...
In previous blog posts, we have written about indicators generated from surveys that we run through Facebook and Google. In addition to these, we have also generated indicators from electronic medical records. We received access to hospital records and insurance claims data from healthcare partners covering 10-15% of the population of the United States. We have created two indicators from this data:

* "Doctor Visits": percentage of office visits due to COVID-like illness.
* "Hospital Admissions": percentage of hospital admissions due to COVID-19.
...
We calculate the percent of COVID-like illness (%CLI) as (COVID-like + Flu-like + Mixed - Flu) / Total. We subtract the Flu counts because they are unrelated to coronavirus. The indicator is available at four geographic levels: county, MSA, HRR, and state. See [here](https://cmu-delphi.github.io/delphi-epidata/api/covidcast-signals/doctor-visits.html) for more details.
...
Interestingly, certain types of claims have a longer latency than others. We found that for a given date, as more claims are available, the %CLI generally increases substantially. Covid-like claims have a longer latency and thus our early estimates of the indicator underestimate the final value.

### Weekday Effects
Another challenge is the influence of the day of the week on %CLI. On weekends, both total counts and COVID-like counts decrease, but proportionally, total counts decrease more. This is because people are more likely to delay seeking medical care for non-acute issues than acute issues. The total counts include many visits related to non-acute issues, but almost all COVID-like counts are due to acute issues. Without correcting for this weekday effect, the %CLI curve has a "sawtooth" pattern, spiking on weekends. We derived a method to create a corrected signal that accounts for the weekday effect. We assume that the log counts form a smooth underlying curve $\phi$, and a different additive effect $\alpha$ for each day of the week. We have the following formula for the expected counts in a given location $i$: $$\log \mu_{it} = \phi_{it} + \alpha_{wd(t)},$$ where $\mu_{it} = \mathbb{E}[Y_{it}]$ and $wd(t)$ is the day of the week of time $t$. We fit $\alpha$ (and $\phi$) by minimizing the following convex objective function: $$f(\alpha, \phi | \mu) = \sum_{i} -\log \ell(\alpha,\phi_i|\mu_i) + \lambda \|\Delta^3 \phi_i\|_1,$$ where $\ell$ is the Poisson log likelihood and $\Delta^3 \phi$ is the third differences of $\phi$. Once we have fit $\alpha$, we calculate the adjusted counts as $$\dot{Y}_{it} = Y_{it}/\alpha_{wd(t)}.$$ As mentioned above, we need to fit different $\alpha$ parameters for each type of count. Below, we see the effect of making these corrections below. When we do not correct for the weekday effect, we see a sawtooth pattern that clearly does not represent true changes in COVID-like illness within a location. However, when we make the weekday correction, we get a smooth curve that looks reasonable. It is important to note that this correction is not time-wise smoothing! Rather, we are making a point-wise correction based on the fitted $\alpha$ parameters.
