...
ggplot(inner_join(dv_cases_df,lm_dv_cases_df, by="geo_value") %>%
  ...
  geom_line(aes(time_value,corr_in,color="Single Correction (in sample)")) +
...
Above, we suggested that the condition of the pandemic should be represented in the DV signal earlier than the case incidence. We can test this by calculating the time-wise correlation between the DV signal and cases at different time shifts. Let $c_{it}$ and $d_{it}$ be the confirmed COVID-19 rate and DV indicator respectively, at location $i$ and time $t$. The $k$-day shifted correlation at location $i$ from time $t_1$ to $t_2$ is $cor(c_{i,t_1:t_2}, d_{i,t_1+k:t_2+k})$. If the DV signal is indeed a leading indicator relative to case incidence, then the shifted correlation should be highest for some negative value of $k$. We see below that this is indeed the case! The mean $-2$ and $-1$ day shifted correlations are higher than the non-shifted correlation (highest for lag $-1$). This suggests that the DV signal may precede case incidence by an average of one day. Although this is modest, every day counts when it comes to combating a pandemic. Additionally, the shifted correlation of the DV signal and death incidence is highest when the DV signal is shifted at least three weeks earlier.

```{r, message=FALSE, warning=FALSE}
full_counties = (dv_cases_df %>% group_by(geo_value) %>% summarize(days_present=n()) %>% filter(days_present==max(days_present)))[["geo_value"]]

rank_cor_lag = function(x,y,lag) {
  cor(x[lag+1:length(x)],y[1:(length(y)-lag)],method="spearman",use="na.or.complete")
}
dv_cases_df = dv_cases_df %>% arrange(time_value)
plot_df = NULL

df_deaths = suppressWarnings(covidcast_signal("usa-facts", "deaths_7dav_incidence_prop", start_day, end_day))

dv_deaths_df = bind_rows(data_adjusted, df_deaths) %>%
  filter(geo_value %in% geo_values) %>%
  select(geo_value, signal, time_value, value) %>%
  tidyr::pivot_wider(names_from=signal,values_from=value) %>%
  rename(deaths=deaths_7dav_incidence_prop, dv=smoothed_adj_cli) %>%
  filter(purrr::map_lgl(geo_value,function(fips) { substr(fips,3,5) != "000"})) %>%
  group_by(time_value) %>%
  ungroup()

for (lag in 0:21) {
  d1 = dv_cases_df %>% filter(geo_value %in% full_counties) %>%
    group_by(geo_value) %>% summarize(corr_cases = rank_cor_lag(dv, cases, lag))
  d2 = dv_deaths_df %>% filter(geo_value %in% full_counties) %>%
    group_by(geo_value) %>% summarize(corr_deaths = rank_cor_lag(dv, deaths, lag))
  d = inner_join(d1, d2, by="geo_value")
  d[["lag"]] = lag
  plot_df = bind_rows(plot_df,d)
}
for (lag in (-21:-1)) {
  d1 = dv_cases_df %>% filter(geo_value %in% full_counties) %>%
    group_by(geo_value) %>% summarize(corr_cases = rank_cor_lag(cases, dv, -lag))
  d2 = dv_deaths_df %>% filter(geo_value %in% full_counties) %>%
    group_by(geo_value) %>% summarize(corr_deaths = rank_cor_lag(deaths, dv, -lag))
  d = inner_join(d1, d2, by="geo_value")
  d[["lag"]] = lag
  plot_df = bind_rows(plot_df,d)
}

ggplot(plot_df %>% group_by(lag) %>% summarize(mean_cor_cases=mean(corr_cases,na.rm=TRUE), mean_cor_deaths=mean(corr_deaths,na.rm=TRUE))) +
  geom_line(aes(x=lag,y=mean_cor_cases,color="Case Incidence")) +
  geom_line(aes(x=lag,y=mean_cor_deaths,color="Death Incidence")) +
  labs(x="Days of DV Shift", y="Correlation", title="Mean Time-wise Correlation of Time-Shifted DV signal to Cases and Deaths", subtitle="Over all counties with DV signal present every day from April 15 to October 1")
```
...
We believe that this dataset will be useful for forecasting Covid cases and deaths, since a doctor visit usually precedes a confirmed test and death. As shown above, the DV signal is a leading indicator relative to cases and deaths. Forecasting models can also take the current DV signal as a feature to help predict what the case incidence will be several weeks into the future.
...
*[Roni Rosenfeld](http://cs.cmu.edu/~roni) is a lead researcher in the Delphi 
group, and a Professor and Head of the Machine Learning Department at CMU. He
is also a Google Fellow.*
