...
ggplot(tibble(thresholds = seq(500,20000,by=500)) %>%
         mutate(avg_corr = purrr::map_dbl(thresholds, function(t) { mean(df_time_cor %>% filter(cases >= t) %>% pull(value), na.rm=T)}))) +
...
ggplot(inner_join(df_space_cor %>% rename(orig_cor = value), df_space_corrected_cor %>% rename(corrected_cor = value))) +
...
We would also like to see how the accuracy and coverage of EMR-based signals can be increased when data coverage increases dramatically. As mentioned above, we have received access to data from Change Healthcare, which when fully extracted will increase our coverage five fold -- to over 50% of the U.S. population. We would be interested in comparing Change's signal to our current signal in sample size, correlations, and behavior. Some insurance providers cover different populations with different rates, which could make their signal more or less correlated with the reported case incidence. We would expect that by combining datasets and dramatically increasing coverage we can produce a significantly more useful signal.
