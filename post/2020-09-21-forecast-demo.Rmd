...
Building on our previous two posts (on our COVID-19 symptom surveys 
through [Facebook](https://delphi.cmu.edu/blog/2020/08/26/covid-19-symptom-surveys-through-facebook/)
and [Google](https://delphi.cmu.edu/blog/2020/09/18/covid-19-symptom-surveys-through-google/)),
this post offers a more technical deep dive, including detailed empirical analysis
and an examinination of whether the % CLI-in-community indicators from our two surveys 
can be used to improve the accuracy of short-term forecasts of county-level COVID-19 case rates.
...
**Forecasting** long been a primary initiative of the Delphi project
...
At the outset, we should state that this post is neither a demo 
of our current forecasters no an authoritative take on cutting-edge forecasting. 
Instead, our purpose here to study the Facebook and Google % CLI-in-community signals,
demonstrating their value, when used as features, to add predictive power
beyond what we can achieve with (fairly simple) time series models trained on case rates alone.
In a future blog post, we'll follow up with details on our "production" forecasters. 

In this post, when we talk abotu forecasting, we mean the task of predicting future case rates directly. 
While we focus here on forecasting, there are a number of other important prediction problems 
that arise as part of pandemic response.
For example, we also work on **hotspot detection**,
where the goal is to predict *whether case rates will rise significantly*,
rather than to predict the future case rates directly. 
Stay tuned into the Delphi blog for a post on hotspot detection soon.

To motivate why we might be optimistic about the utility of our survey signals
as incorporated into forecasting models, you might want to furst check out
our [previous exploratory investigations](https://delphi.cmu.edu/blog/2020/08/26/covid-19-symptom-surveys-through-facebook/#some-interesting-examples), 
which suggested that our survey signals might serve as **early indicators**
of COVID-19 activity, and hence could help with hotspot detection. 
...
Formaly, our goal here is to predict county-level 
...
is to predict the *smoothed* COVID-19 case incidence rate 7 days ahead and 14 days ahead, 
where the smoothing is performed via a 7 day trailing average.
...
rate for location (county) $\ell$ and time (day) $t$. 
...
The third model is exactly same but substituting the Google signal 
...
(to predict case rates for time $t_0+d$),
we fit a linear model using least absolute deviations (LAD) regression, 
training over all $\ell$ (all 440 counties), 
and all $t$ that are within the most recent 14 days of data 
...
For an error metric, we consider **scaled absolute error** (or just scaled error for 
short):
$$
\frac{|\hat{Y}_{\ell,t_0+d} - Y_{\ell,t_0+d}|}
{|Y_{\ell,t_0} - Y_{\ell,t_0+d}|},
$$
where the error in the denominator is the error of the *straw-man* model,
...
over the straw man's error (so numbers like 0.8 or 0.9 would be favorable, 
...
in forecasting difficulty, and normalizing by the straw man's
...
one day at a time (from April 11 to September 1), 
fits the four models, makes 7 and 14 day ahead predictions, and records errors. 
It takes a little while to run[^1], the culprit being LAD regression: 
here the training sets get moderately large 
(aggregating the data over 440 counties and 14 days results in over 6000 training samples), 
and at this scale LAD regression is much slower than least squares regression. 
We ran this R code separately and saved the results in an RData file; 
you can find this in the [same GitHub 
...
but as we explained in our last post,we [plan to bring it back](https://delphi.cmu.edu/blog/2020/09/18/covid-19-symptom-surveys-through-google/#google-survey-redux) later this fall.) 
Hence we skip studying the 14 day ahead forecasts results in this four-way model discussion, 
as they're only based on 2 days of test data.
...
of the model's forecast relative to that of the straw man; and each test day 
...
We can also see that the median scaled errors areall close to 1 
...
that they probably are: below we conduct a [sign test](https://en.wikipedia.org/wiki/Sign_test)[^2] 
for whether the difference in the "Cases" model's scaled error 
and each other model's scaled error is centered at zero. 
The sign test is run on the 9 test days x 440 counties = 3960 pairs of scaled errors. 
The p-values from the "Cases" versus "Cases + Facecbook" 
...
whereas the sign test does not, and thus we report results from the latter.
...
Next we focus on comparing results between the "Cases" and "Cases + Facebook" models only. 
Restricting to a common available forecast dates yields a much longer test period, 
May 6 through August 25 for 7 day ahead forecasts, 
...
Now we see a decent improvement in median scaled error for the "Cases + Facebook" model, 
and this is true for both 7 and 14 day ahead forecasts.
...
between the median scaled errors from 
...
can improve the accuracy of short-term forecasts 
of county-level COVID-19 case incidence rates. 
The statistical significance associated with these improvements 
is more apparent with the Facebook signal, 
thanks to the much longer test period available. 
With either signal, of the magnitude of the improvement offered seems modest but nontrivial, 
especially because the forecasting problem is so hard in the first place.

We reiterate that this was just a demo. Our analysis was fairly simple and lacks
a few qualities that we'd expect in a truly comprehensive, realistic forecasting analysis. 
To name three: 
...
3. The way we trained our forecasts does not account 
...
and go through multiple revisions (past data values get updated as time goes on).
...
play with [our COVIDcast API](https://cmu-delphi.github.io/delphi-epidata/api/covidcast.html)
via our Python and R packages,
and if you're feeling adventurous, 
consider competing in the [COVID-19 Symptom Data Challenge](https://www.symptomchallenge.org/),
and trying your hand at developing novel analytic approaches to extracting insights
from our Facebook symptom survey data. 
