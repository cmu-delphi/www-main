...
We have made our code and data available to the public since the quaint pre-COVID years, when our group worked primarily on flu forecasting.
The [Epidata API](https://cmu-delphi.github.io/delphi-epidata/) 
was initially created to share data from our numerous influenza surveillance streams. Now that COVID-19 is here, our collaborations with partner organizations in technology and healthcare enable us to include a number of exclusive signals which would not otherwise be publicly available. We have redoubled our commitment to open code and data in our COVID-19 response efforts with the launch of the COVIDcast API, providing researchers and decision-makers 
...
For each use case, researchers need quick access to reliable, up-to-date data streams. The COVIDcast API can provide that access.
...
* Quidel COVID antigen tests:
  ...
  available to us in early May. This data source fills an important gap because many public testing data sources only include
  PCR tests, not antigen tests.
...
* Google search trends:
  ...
  which emerged as a symptom of COVID-19. 
...
The first step of using the packages to acquire the data is to identify the source and signal name for the data you want to analyze. Suppose, for example, that you browse the
[COVIDcast signal
...
R and Python are not required for access to our data; users can also make
HTTP requests to the API directly and receive data back in JSON format. By
...
expertise to ingest and serve data on the COVIDcast API, including Taylor Arnold, Logan Brooks, Eu Jing Chua, Addison Hu, Sangwon Hyun, Maria Jahja, 
Jimi Kim, Zachary Lipton, Natalia Lombardi de Oliveira, Lester Mackey, Pratik Patil, Samyak Rajanala, Roni Rosenfeld, Aaron Rumack, James Sharpnack, Noah Simon, Jingjing Tang, Robert and Ryan Tibshirani, Valérie Ventura, Larry Wasserman, and Jeremy Weiss.*
