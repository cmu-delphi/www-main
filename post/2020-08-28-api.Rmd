...
We have made our code and data available to the public since the
quaint pre-COVID years, when our group worked primarily on flu
forecasting.  The [Epidata
API](https://cmu-delphi.github.io/delphi-epidata/) was initially
created to share data from our numerous influenza surveillance
...
* **Nowcasting.** If data can provide a clear picture of what's
  happening *right now*, then this picture can be used to make decisions
  about re-opening criteria, resource allocation, closures, and so on.
...
For each use case, researchers need quick access to reliable, up-to-date data
streams. The COVIDcast API can provide that access.
...
* Quidel COVID antigen tests: Quidel is a national provider of networked lab
  testing devices, and began making de-identified records on COVID-19 antigen
...
Jingjing Tang, Robert and Ryan Tibshirani, Valérie Ventura, Larry Wasserman, and
Jeremy Weiss.*
