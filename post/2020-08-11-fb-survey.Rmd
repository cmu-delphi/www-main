...
(enough for us to create meaningful estimates) for an average of nearly 1000
counties per week; and has been taken over 9.5 million times so far. An
...
to be reflective of the true rate of COVID-19 (and they're not intended to be).
...
show up in the % CLI indicator). Let's start by looking at Miami-Dade County,
which experienced a surge of new COVID-19 cases between June and July: below we
plot daily new confirmed COVID-19 cases over time (using a 7-day trailing
average for smoothing) as a blue curve. We also plot the % CLI-in-community
indicator as a red curve. Note that these two curves, case counts and %
CLI-in-community, are not measured in the same units, hence our use of two
y-axes: one on the left for case counts, and one on the right for %
CLI-in-community.
...
plot_one = function(geo_value, title = fips_to_name(geo_value), xlab = NULL,
...
analogous plots below, with COVID-19 cases in red and the % CLI-in-community 
indicator in blue. We can see that the % CLI-in-community indicator is (a) never
lagging and (b) again leading in many counties, for example, Dallas County, 
Orange County, Hidalgo County, and Nueces County are a few notable examples.
...
"active" counties, defined to have had at least 500 confirmed COVID-19 cases in 
total. The Spearman correlation between two vectors is just the usual (Pearson)
applied to the vectors of their ranks. (Spearman correlation is invariant to 
monotone transformations; we can think of it as assessing whether one variable 
is high when another is high, without relying on any particular functional 
form for the dependence between the two variables.) We do the same for the % 
CLI-in-community indicator, and plot the results below, with the % CLI indicator
correlations in red, and the % CLI-in-community correlations in blue. We can see
clearly that the % CLI-in-community indicator produces, consistently across all 
time, *much* higher correlations. Even in an absolute sense, the correlations
from the % CLI-in-community are worth noting: it reaches over 0.8 for a period
...
ggplot(df_cor, aes(x = time_value, y = value)) + 
  geom_line(aes(color = signal)) + 
  labs(title = "Correlation per time, over all counties",
       x = "Date", y = "Correlation") + theme_bw() +
  theme(legend.position = "bottom", legend.title  = element_blank())
```

Another interesting observation is that the correlations from either indicator
witness huge increases sometime around mid June. One explanation: this could
be due to the fact that many counties saw big surges in COVID-19 activity around
that time. These surges created a large spread between the COVID-19 case rates 
...
swamp the noise. Of course, this is really just speculation, and we can't say 
for certain what the real cause is. Some decent empirical evidence for our 
explanation, however, is to look at the median absolute deviation of COVID-19 
case rates as a function of time, as we do below. We can see a very similar 
trend: the median absolute deviation (a robust measure of spread) starts to rise
sharply sometime around mid June.
...
Next we compute, for each "active" county (with at least 500 total cases), the
...
ggplot(df_cor, aes(value)) + 
  ...
  labs(title = "Correlation per county, over all time",
       x = "Correlation", y = "Density") + theme_bw() +
  theme(legend.position = "bottom", legend.title = element_blank())
```

Finally, we can create choropleth maps of these correlations to learn where
(geographically speaking) they're high and where they're not. As we can see, the
% CLI-in-community indicator yields high correlations pretty much throughout the
US, whereas the % CLI indicator is a bit more spotty. (Note that here a gray
color denotes a missing value: either that county had below 500 total COVID-19
cases and was deemed "inactive", or we didn't have enough data from the surveys
in order to estimate % CLI and % CLI-in-community signals there.)
...
# Plot choropleth maps, using the covidcast plotting functionality
p1 = plot(df_cor1, title = "Correlation for % CLI signal", range = c(-1, 1),
          choro_col = cm.colors(10))
p2 = plot(df_cor2, title = "Correlation for % CLI-in-community signal", 
