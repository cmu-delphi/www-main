...
Disseminated by Facebook, our survey reaches roughly 74,000 people every day.
...
some of the exciting new directions that we are pursuing.
...
This is an ongoing operation and our survey reaches 74,000 respondents every day.
Respondents provide information about COVID-related
symptoms, contacts, risk factors, and demographics,
allowing us to examine county-level trends across the US.
We believe that this combination of *detail* and *scale*
...
and visualize it on our [COVIDcast interactive map](https://covidcast.cmu.edu/).
...
Back in March 2020, as Coronavirus were first spiking across the country,
we launched discussions with Facebook about running a survey,
advertised through their site, to collect real-time,
...
to construct a early indicator of COVID activity in the US.
...
Every day since, Facebook directs a random sample of its users to our survey,
hosted on [Qualtrics](https://www.qualtrics.com/).
...
As of this writing, our survey reaches 74,000 respondents per day,
delivering enough data for us to create meaningful estimates
for an average of nearly 1,000 counties per week.
Over the course of the survey, we have already collected over 10 million responses!
An [international version of the survey](https://covidmap.umd.edu/),
available in over 50 languages, was launched soon after by a team at the University of Maryland.

Before going into detail later about our survey and our survey-based indicators,
here are a couple maps to ground your intuition.
...
based on data from [JHUCSSE](https://github.com/CSSEGISandData/COVID-19),
...
We generated these plots using our public-facing [covidcast R
...
of COVID activity in a particular region. 
And to be clear, it's not just that we're looking at symptoms that's important here, 
it's the *way* we're measuring them: 
...
We expect that self-reported symptom data 
shouldn't be subject to the same problems. 

It's also worth being clear about what we're *not* able to say with these
surveys:
...
- Our symptom data is entirely self-reported, (in ccontrast to data
...
Our survey has 4 sections and is 35 questions long. The first section is
short and gathers information on a core set of symptoms used to define a
condition called **COVID-like illness** or **CLI**, which we define as *fever of
at least 100 °F, along with cough, shortness of breath, or difficulty
breathing*. This mirrors the standard definition of influenza-like illness or
...
Now we'll turn to some suggestive data analaysis that provides evidence
that our survey-based CLI signals might be early indicators of COVID activity. 
We'll consider the % CLI-in-community indicator, which tends to be more stable than
the % CLI indicator (which shows similar, but noisier, trends).
...
(first dashed vertical line), which happens *6 days before* COVID-19 cases 
...
at the *recall* of the % CLI-in-community signal. 
Of course, this is only one half of the story: 
for the signal to be a useful early indicator,
we'd also need to know something about its *precision*: 
...
(Spearman correlation assesses whether one variable is high when another is high, 
...
consistently across all time, *much* higher correlations. 
Even in an absolute sense, the correlations from the % CLI-in-community are noteworthy: 
they reach over 0.8 for a period between July and August.
...
This could be because many counties saw big surges in COVID-19 activity around that time. 
These surges created a larger spread between the COVID-19 case rates across the country,
and so county-to-county differences started to become easier to track 
with the indicators, as the magnitude of these differences started to swamp the noise.
...
by looking at how COVID-19 case rates vary between counties over time (see below). 
The median absolute deviation (a robust measure of spread) between counties 
rises sharply sometime around mid-June, 
...
the Spearman correlations between each of our indicators and COVID-19 case rates, across all time. 
We can visualize this in a few different ways.
...
we again see that the % CLI-in-community indicator produces *much* higher correlations.
...
have *many* possible uses beyond simply surveying symptoms. You would be right.
In other sections, our survey contains questions on testing, 
behavior, medical care, mental health, and related topics,
...
To take these efforts to the next level, we will soon release a new version of the survey. 
Based on feedback from other researchers, public health agencies, the University of
...
This new survey is currently being deployed, and data should become available in the next few weeks. 
Detailed data will be available to researchers, while new aggregates---such as of mask-wearing---will be made public, 
as usual, through our [COVIDcast
API](https://cmu-delphi.github.io/delphi-epidata/api/covidcast.html) and
[COVIDcast interactive map](https://covidcast.cmu.edu/).

**Credits.** *Logan Brooks and Katie Mazaitis
