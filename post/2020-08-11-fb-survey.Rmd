...
such a combination of *detail*, *scale* and *timeliness* <!--- alt. *promptness* ---> has never before been available via
surveys in a public health emergency: we collect information on COVID-related symptoms,
...
indicators of COVID-like illness <!--- CLI has not yet been technically defined ---> that are derived from our surveys. In terms of
...
random sample of its users to our survey, hosted on
...
receives responses from an average of over 74,000 users per day<!--- 'responses' is ambigupus and coudl mean answers to individual questions --->; delivers adequate data
...
We'll go into detail later about our survey and our survey-based indicators,
...
point, but much more subtle, and we won't delve into it in this post.)
...
which may be a sizeable fraction of the US population, but certainly not all of it.
...
short and gathers information on a core set of symptoms used to define a condition
...
the working definition of CLI used by the U.S. Centers for Disease Control and Prevention (CDC).

Using data from responses to the survey's first section, we can 
estimate two key quantities, in a given location, on a given day:
...
medical-seeking behavior. The third section gathers information on contacts and risk
factors, and the fourth section gathers information on demographics. These survey
...
<!--- if you use red font for the right-side (%CLI) Y-axis label, and blue font for the left-side (Cases) Y-axis label,
it will help parse the picture more quickly.  You could also use these colors for the vertical dashed lines. --->
...
and (b) leading in many counties; for example, Dallas County, Orange
...
correlation between the % CLI indicator and COVID-19 case rates, across the NUMBER <!--- provide number here --->
counties that had at least 500 confirmed COVID-19 cases in total. (The Spearman
...
CLI-in-community are noteworthy: they reach over 0.8 for a period between July
and August.
<!--- since you used these particular red and blue colors for %CLI-in-community vs. Confired Cases, it may be better to use a different color scheme to compare %CLI to %CLI-in-community, in the section below.  Or, in the above analysis, use black for Confirmed Cases, and keep chromatic colors for our indicators.  --->   
...
ggplot(df_cor, aes(x = time_value, y = value)) + 
  ...
  labs(title = "Correlation between CLI signals and cases",
       subtitle = sprintf("Over the NUMBER counties with at least %i cumulative cases", case_num),
...
ggplot(df_in_act %>% group_by(time_value) %>% 
  ...
  labs(title = "Median absolute deviation in COVID-19 case rates",
       subtitle = sprintf("Over all counties with at least %i cumulative cases", case_num),
...
<!--- 'cumulative' might be slightly preferred to 'total'.  Either is fine, but you should stick to just one of them throughout --->
the
...
of the data (correlations by county, rather than by day), we again see
...
total COVID-19 cases, or we didn't have enough data 
...
right. As we explain above, beyond the first section, the survey contains
many questions on testing, behavior, medical care, mental health, and
...
groups sought and received access to our survey data, and are using it to study various aspects
