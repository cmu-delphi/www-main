  ...
  return(ggplot(df, aes(x = time_value, y = value)) +
           ...
           scale_y_continuous(name = ylab1, limits = range1,
                              sec.axis = sec_axis(trans = trans12, 
                                                  name = ylab2)) + 
           labs(x = xlab, title = title) + theme_bw() +
...
periods where new COVID-19 cases remain flat. We'll save a formal
precision-recall analysis for future work. 

## Basic Correlation Analysis

To complement the more exploratory, qualitative analysis of the last section, 
we'll conduct a very simple quantitative analysis here, by computing some basic
measures of correlation between our survey-based indicators and confirmed 
...
geo_values = covidcast_signal("jhu-csse", "confirmed_cumulative_num",
                              ...
                              geo_type = "county") %>%
  group_by(geo_value) %>% summarize(total = sum(value)) %>% 
...
# Stack rowwise into one data frame, then plot
...
# Stack rowwise into one data frame, then plot a histogram
df_cor = rbind(df_cor1, df_cor2)
df_cor$signal = c(rep("% CLI", nrow(df_cor1)), 
                  rep("% CLI-in-community", nrow(df_cor2)))
...
df_cor1$direction = df_cor2$direction = 0
