---
title: Large Causal Inference for COVID-19
...
summary: |
  The goal is to describe some work we have been doing on causal inference.
acknowledgements: |
...
The goal is to describe some work
we have been doing on causal inference.
The details
will be available
in two papers that we expect
to complete soon.
The first paper deals with mobility and the second
deals with mask usage.
Here we will aim to give the high level ideas
...
that reduced social mobility
will lead to fewer deaths.
But how do we quantify this effect?
As everyone knows,
correlation is not causation,
and so we have to use rigorous causal inference methods.
...
$$
\E[Y^a] = \int \mu(x,a) p(x) dx = \E[\mu(X,a)]
$$
...
$$
\hat{\E(Y^a)} = \frac{1}{n}\sum_i \hat\mu(X_i,a).
$$
...
How do we
find the mean $\E[Y^{\overline{a}_t}_t]$?
As a general rule,
...
The correct way to
get the mean of $\E[Y^{\overline{a}_t}]$
...
\int\cdots\int
\E[Y_t | \overline{A}_t = \overline{a}_t, \overline{Y}_{t-1}=\overline{y}_{t-1}]
...
$\E[Y^{\overline{a}_t}_t]$ by
$\E[Y_t| {\rm do}(\overline{a}_t)]$.
...
that $\E[Y^{\overline{a}_t}]\approx \beta_0 + \beta \sum_s a_s$.
...
$$
\E[L_t^{\overline{a}_t}]= s(t) + \beta M_t
$$
...
$$
\int\cdots\int
\E[L_t | \overline{A}_t = \overline{a}_t, \overline{L}_{t-1}=\overline{\ell}_{t-1}]
...
\begin{center}
\includegraphics[scale=.8]{Matteo_plot1.pdf}
\end{center}

\begin{center}
\includegraphics[scale=.8]{Matteo_plot2.pdf}
\end{center}
...
$\overline{a}_T = (A_1+\gamma,\ldots, A_T+\gamma)$.
In other words,
...
\begin{center}
\includegraphics[scale=.8]{plot_for_blog2_bigstates}
\end{center}

\begin{center}
\includegraphics[scale=.8]{plot_for_blog2}
\end{center}
...
as possible.
So far we use all
past vaalues of death and mobility as
cconfounding variables.
But of course,
